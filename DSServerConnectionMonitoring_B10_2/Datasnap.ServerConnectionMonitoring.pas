...
uses System.Classes,
     ...
     Datasnap.DSService,
     Data.DBXPlatform, Datasnap.DSSession;
...
type
  ...
  public
    ...
    property ProtocolStr: string read FProtocolStr;
...
begin
  if (Self.ComponentCount > 0) and (List <> nil) then
  begin
    Count := Self.ComponentCount - 1;
    for Index := 0 to Count do
    begin
      ServerComponent := Self.Components[Index];
...
begin
  ...
  if (FProtocol = pTCPIP) then
    FProtocolStr := 'TCP/IP'
  else if (FProtocol = pHTTPrest) then
    FProtocolStr := 'HTTP Rest'
  else
    FProtocolStr := 'HTTP Tunnel';
